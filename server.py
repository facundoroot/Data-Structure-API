...
#configure sqlite3 to inforce foreign key contraints
...
def get_all_users_ascending():
    ...
    all_users_ll = linked_list.LinkedList() 
    for user in users:
        all_users_ll.insert_at_the_end(
            {
                "id": user.id,
                "name": user.name,
                "email": user.email,
...
def get_one_blog_post(blog_post_id):
    ...
    #if a insert ascending or descending ill get an unbalanced tree like similar to a linked list
    # so i will insert randomly
...
def get_numeric_post_bodies():
    # iterate through each letter of the ody converting to numbers
    blog_posts = BlogPost.query.all()
